...
namespace
{
	...
	static void* ExpandChannelToCanvas(const Document* document, Allocator* allocator, Layer* layer, Channel* channel)
	{
		...
		if (document->bitsPerChannel == 8)
			return ExpandChannelToCanvas<uint8_t>(allocator, layer, channel->data, layerWidth, layerHeight);
		else if (document->bitsPerChannel == 16)
			return ExpandChannelToCanvas<uint16_t>(allocator, layer, channel->data, layerWidth, layerHeight);
		else if (document->bitsPerChannel == 32)
			return ExpandChannelToCanvas<float32_t>(allocator, layer, channel->data, layerWidth, layerHeight);
...
//                     const unsigned int layerWidth = layer->right - layer->left;
//                     const unsigned int layerHeight = layer->bottom - layer->top;
//                     const void* rawR = layer->channels[indexR].data;
//                     const void* rawG = layer->channels[indexG].data;
//                     const void* rawB = layer->channels[indexB].data;
//                     const void* rawA = layer->channels[indexA].data;
//                     // 3. 创建一个新的、仅包含图层自身数据的、裁剪后的图像缓冲区
// 					uint8_t* trimmedImage = CreateInterleavedImage<uint8_t>(&allocator, rawR, rawG, rawB, rawA, layerWidth, layerHeight);
//                     std::wstringstream filenamePng;
// 					filenamePng << GetSampleOutputPath();
// 					filenamePng << L"layer";
// 					filenamePng << layerName.str();
// 					filenamePng << L".png";
// 					tgaExporter::SavePNG(filenamePng.str().c_str(), layerWidth, layerHeight, channelCount,(const uint8_t*)trimmedImage);
