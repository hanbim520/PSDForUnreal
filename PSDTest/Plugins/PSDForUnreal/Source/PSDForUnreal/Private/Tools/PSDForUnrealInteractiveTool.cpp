...
#include "PSDForUnrealInteractiveTool.h"
...
#define LOCTEXT_NAMESPACE "UPSDForUnrealInteractiveTool"

/*
 * ToolBuilder
 */

UInteractiveTool* UPSDForUnrealInteractiveToolBuilder::BuildTool(const FToolBuilderState & SceneState) const
{
	UPSDForUnrealInteractiveTool* NewTool = NewObject<UPSDForUnrealInteractiveTool>(SceneState.ToolManager);
...
UPSDForUnrealInteractiveToolProperties::UPSDForUnrealInteractiveToolProperties()
{
...
void UPSDForUnrealInteractiveTool::SetWorld(UWorld* World)
{
	check(World);
...
void UPSDForUnrealInteractiveTool::Setup()
{
	...
	Properties = NewObject<UPSDForUnrealInteractiveToolProperties>(this, "Measurement");
...
void UPSDForUnrealInteractiveTool::OnUpdateModifierState(int ModifierID, bool bIsOn)
{
...
FInputRayHit UPSDForUnrealInteractiveTool::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
...
void UPSDForUnrealInteractiveTool::OnClickPress(const FInputDeviceRay& PressPos)
{
...
void UPSDForUnrealInteractiveTool::OnClickDrag(const FInputDeviceRay& DragPos)
{
	UpdatePosition(DragPos.WorldRay);
}


FInputRayHit UPSDForUnrealInteractiveTool::FindRayHit(const FRay& WorldRay, FVector& HitPos)
{
...
void UPSDForUnrealInteractiveTool::UpdatePosition(const FRay& WorldRay)
{
...
void UPSDForUnrealInteractiveTool::UpdateDistance()
{
	Properties->Distance = FVector::Distance(Properties->StartPoint, Properties->EndPoint);
}


void UPSDForUnrealInteractiveTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
...
void UPSDForUnrealInteractiveTool::Render(IToolsContextRenderAPI* RenderAPI)
{
